...
@dp.callback_query_handler(years_callback.filter(item_name = 'bmu_2019'))
async def buying_corse(call: CallbackQuery, callback_data: dict):
    await call.message.answer('bmu_2019')
    await call.message.delete()

@dp.callback_query_handler(years_callback.filter(item_name = 'bmu_2020'))
async def buying_corse(call: CallbackQuery, callback_data: dict):
    await call.message.answer('bmu_2020')
    await call.message.delete()

@dp.callback_query_handler(years_callback.filter(item_name = 'bmu_2021'))
async def buying_corse(call: CallbackQuery, callback_data: dict):
